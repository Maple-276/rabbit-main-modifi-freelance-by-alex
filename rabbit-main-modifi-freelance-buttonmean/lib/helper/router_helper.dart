...
class RouterHelper {
  ...
  static const String phoneRegistrationScreen = '/phone-registration';
  ...
  static String getOtpRegistrationScreen(String tempToken, String userInput, {String? userName, RouteAction? action}) => _navigateRoute('$otpRegistration?temp_token=$tempToken&user_input=$userInput${userName != null ? '&user_name=$userName': ''}', route: action);
  static String getPhoneRegistrationRoute({RouteAction? action}) => _navigateRoute(phoneRegistrationScreen, route: action);
  ...
  static String getHomeItemRoute(String type, ProductType? productType) => _navigateRoute('$homeItem?type=${Uri.encodeComponent(type)}&product_type=${productType?.name}');
  ...
  static final goRoutes = GoRouter(
    ...
    routes: [
      ...
      GoRoute(path: searchResultScreen, builder: (context, state) => _routeHandler(
        context, path: _getPath(state), SearchResultScreen(searchText: jsonDecode(state.uri.queryParameters['text'] ?? '')), isBranchCheck: true,
      )),
