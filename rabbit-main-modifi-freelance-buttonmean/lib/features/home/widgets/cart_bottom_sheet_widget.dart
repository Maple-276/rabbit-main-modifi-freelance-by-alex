                  ...
                  if(variationList != null){
                    for(int index = 0; index < variationList.length; index++) {
                      if(!variationList[index].isMultiSelect! && variationList[index].isRequired!
                        ...
                        showCustomSnackBarHelper('${getTranslated('choose_a_variation_from', context)} ${variationList[index].name}', isToast: true, isError: true, type: SnackBarType.error, showProgressBar: true);
                        return;
                        ...
                        showCustomSnackBarHelper('${getTranslated('you_need_to_select_minimum', context)} ${variationList[index].min} '
                            '${getTranslated('and_maximum', context)} ${variationList[index].max} ${getTranslated('for', context)} ${variationList[index].name}',isToast: true, isError: true, type: SnackBarType.error, showProgressBar: true);
                        return;
                      }
                    }
                  }
      ...
      InkWell(
        onTap: () {
          ...
          if(productProvider.checkStock(
            ...
            showCustomSnackBarHelper(getTranslated('out_of_stock', context), type: SnackBarType.error, showProgressBar: true);
          }
        },
        child: Container(
          padding: const EdgeInsets.all(Dimensions.paddingSizeExtraSmall),
